...
public class ProgressivePrinter {
    ...
    public void pauseWithDelay(int millis) {
        delay(millis);
        pause();
    }
    public void pause() {
        // Instantaneous
        printStream.printf("%nPress [Enter] to continue.");
        consolePauser.nextLine();
    }
    public synchronized void delay(long millis) {
        try {
            wait(millis);
        }
        catch (InterruptedException waitWasInterruptedException) {
            waitWasInterruptedException.printStackTrace(System.err);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        progPrinter.pauseWithDelay(2000);
